...
function onSystemMessage(msg, type, sender)
    ...
    if type == TAG.TYPE_ERROR then
      if msg:match("^.-:\\.+: _FORCE_KILL_THE_SCRIPT_.+") then fKilledScr = sender; type = TAG.TYPE_INFO; msg = "Force killing the script..."
      elseif fKilledScr then
        if fKilledScr == sender then
          local scr = msg:match("^Script died due to an error%. %((.+)%)$")
          if scr then type = TAG.TYPE_INFO; msg = string.format("Force killed the script. (%s)", scr) end
        end
        fKilledScr = nil
      end
    end
